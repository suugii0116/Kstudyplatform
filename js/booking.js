...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const localStorageKey = 'kstudyBookingDraft';
  const fieldErrorMap = {
    'book-name': 'name-error',
    'book-email': 'email-error',
    'book-phone': 'phone-error',
    'book-program-type': 'program-error'
  };
  ...
  function validateStep(stepIndex) {
    ...
    activeStep.querySelectorAll('input, select, textarea').forEach(el => el.classList.remove('input-error'));
  ...
  function setStatusMessage(message, type = 'info') {
    ...
    statusMessage.className = `booking-status-message ${type}`;
  }
  ...
  function persistFormState() {
    ...
    const draft = {
      ...
      values: {
        ...
        currentLevel: fields.currentLevel?.value || '',
        programType: fields.programType?.value || '',
        targetTerm: fields.targetTerm?.value || '',
        koreanLevel: fields.koreanLevel?.value || '',
  ...
  function restoreFormState() {
    ...
    try {
      ...
      if (typeof draft.currentStepIndex === 'number') {
        currentStepIndex = Math.min(Math.max(draft.currentStepIndex, 0), steps.length - 1);
      }
    } catch (error) {
  ...
  function populateSummary() {
    ...
    if (summaryName) summaryName.textContent = fields.fullName.value.trim();
    if (summaryEmail) summaryEmail.textContent = fields.email.value.trim();
    if (summaryPhone) summaryPhone.textContent = fields.phone.value.trim();
    ...
    if (summaryNotes) {
      ...
      const notesRow = summaryNotes.closest('.summary-data-item');
      if (noteText.length > 0) {
        summaryNotes.textContent = noteText;
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      if (validateStep(currentStepIndex)) {
        ...
        setStatusMessage('Та формын алдааг засна уу.', 'error');
      }
    });
  }
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', (e) => {
      ...
      setStatusMessage('Таны хүсэлт илгээж байна...', 'info');
